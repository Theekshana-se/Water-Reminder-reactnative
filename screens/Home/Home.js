...
import { StyleSheet, Text, View, TouchableOpacity, FlatList, Animated, Dimensions, BackHandler, PanResponder } from "react-native";
...
import { fetchUserWaterConsumption, updateUserWaterConsumption } from '../../lib/appwrite';
...
const Home = ({ userId }) => {
  ...
  const fetchWeatherData = async () => {
    try {
      ...
      const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m`; // Fixed typo
    ...
    const fetchUserDailyGoal = async () => {
      try {
        ...
        if (!accountId) {
          ...
          setDailyGoal(2000);
          setBaseDailyGoal(2000);
          await setBaseDailyGoalOnRegistration(2000);
          await updateUserWaterConsumption(0, 2000);
          return;
        }

        const waterData = await fetchUserWaterConsumption(accountId);
        let baseGoal = waterData?.dailyGoal || 2000;
        let currentConsumption = waterData?.waterConsumption || 0;

        if (!waterData || !waterData.dailyGoal) {
          const weight = await getItem(WEIGHT_KEY);
          const activity = await getItem(ACTIVITY_KEY);
          baseGoal = calcDailyGoal(weight, activity) || 2000;
        ...
        await updateUserWaterConsumption(currentConsumption, adjustedGoal);
      } catch (error) {
        ...
        setBaseDailyGoal(calcDailyIntake);
        await setBaseDailyGoalOnRegistration(calcDailyIntake);
        await updateUserWaterConsumption(0, calcDailyIntake);
      }
    };
  ...
  const confirmModalHandler = async (selectedQuantity) => {
    if (selectedQuantity && dailyGoal) {
      ...
      await updateUserWaterConsumption(newConsumption, dailyGoal);
    }
  };
  ...
  const resetWaterProgress = async () => {
    ...
    await updateUserWaterConsumption(0, dailyGoal);
  };
